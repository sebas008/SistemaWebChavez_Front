...
export class Partidas implements OnInit {
  ...
  load() {
    ...
    this.api.get<PartidaDto[]>('/maestros/partidas').subscribe({
      next: (data) => { this.rows = (data || []).map((x:any) => ({ ...x, activo: x?.activo !== false })); this.loading = false; this.forceRender(); },
      error: (e) => { this.loading = false; this.error = e?.message || 'No se pudo cargar.'; this.notify.error(this.error); this.forceRender(); },
    });
  }

  openNew() { this.editing = null; this.form = { nombre: '', activo: true }; this.modalOpen = true; }
  openEdit(row: PartidaDto) { this.editing = row; this.form = { nombre: row.nombre, activo: !!row.activo }; this.modalOpen = true; }
  ...
  save() {
    ...
    if (!this.editing) {
      this.api.post<any>('/maestros/partidas', { nombre: this.form.nombre.trim() }).subscribe({
        next: () => { this.notify.success('Partida guardada.'); this.closeModal(); this.load(); },
    ...
    this.api.put<any>(`/maestros/partidas/${this.editing.idPartida}`, {
      ...
      next: () => { this.notify.success('Partida actualizada.'); this.closeModal(); this.load(); },
